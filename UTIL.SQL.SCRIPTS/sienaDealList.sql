SELECT        dbo.Deals.SienaReference, dbo.Deals.CustomerSienaView, dbo.Deals.Status, dbo.Deals.StartDate AS ValueDate, dbo.Deals.MaturityDate, dbo.Deals.ContractNumber, dbo.Deals.ExternalReference, dbo.Deals.Book,
                         ...
                         dbo.Deals.ReceiptInstruction, dbo.Deals.NIName, { fn CONCAT(dbo.Deals.DealtCcy, dbo.Deals.AgainstCcy) } AS CCYPair, ISNULL(NULLIF (dbo.Deals.NIName, NULL), CONCAT_WS('', NULL, dbo.Deals.DealtCcy, NULL, 
                         dbo.Deals.AgainstCcy)) AS Instrument, dbo.Portfolio.Description1 AS PortfolioName, dbo.DealRevaluation.Date AS RVDate, dbo.DealRevaluation.MarkToMarket AS RVMTM, dbo.Deals.CounterBook,
                         Book_1.FullName AS CounterBookName, ISNULL(NULLIF (dbo.Deals.CounterBook, NULL), dbo.Deals.CustomerSienaView) AS Party, ISNULL(NULLIF (Book_1.FullName, NULL), dbo.Deals.CustomerSienaView)
                         AS PartyName
